...
namespace ImprovedTransportManager.Overrides
{
    public class PublicTransportAtRoadAIOverrides : Redirector, IRedirectable
    {
        ...
        public static bool PreArrivingToDestination(ushort vehicleID, ref Vehicle vehicleData, VehicleAI __instance)
        {
            ...
            if (!(
            (__instance is BusAI && ITMCitySettings.Instance.expressBuses)
            || (__instance is TramAI && ITMCitySettings.Instance.expressTrams)
            || (__instance is TrolleybusAI && ITMCitySettings.Instance.expressTrolleybus)
            ))
            {
            ...
            ITMLineUtils.GetQuantityPassengerWaiting(currentStop, out int residents, out int tourists, out _);
        ...
        private static bool CheckDespawn(ushort vehicleID, ref Vehicle vehicleData, bool isEmpty, bool forceIfNotEmpty)
        {
            ...
            {                
                ...
                if (currentVehicleCount > targetVehicleCount || !ITMTransportLineSettings.Instance.IsInfoAllowedToLine(vehicleData.Info, vehicleData.m_transportLine))
                {
                    ...
                    {
                        vehicleData.DoSoftDespawn(vehicleID);
                        return false;
                    }
                }
            }
